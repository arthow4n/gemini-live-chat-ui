...
class GeminiLiveService : Service() {
    ...
    private suspend fun executeGeminiQuery(threadId: Long, userAudioFile: File) {
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                val expectedLanguage = repository.settingsStore.expectedLanguage
    ...
    private fun showForegroundNotificationWithTitle(title: String) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            val hasRecordPermission = ContextCompat.checkSelfPermission(
                this@GeminiLiveService,
                android.Manifest.permission.RECORD_AUDIO
            ) == PackageManager.PERMISSION_GRANTED

            if (isRecording.value && hasRecordPermission) {
    ...
    private fun showForegroundNotification() {
        // 1. Immediately call startForeground synchronously to satisfy Android's foreground service policy
        showForegroundNotificationWithTitle("Voice Session")

        // 2. Resolve actual title asynchronously in a coroutine
        serviceScope.launch {
            val title = activeThreadId.value?.let { repository.getThreadById(it)?.title }
            if (title != null) {
                showForegroundNotificationWithTitle(title)
            }
        }
    }
